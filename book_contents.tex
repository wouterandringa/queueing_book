...
With the tools developed up to now we can simulate queueing processes, but we cannot easily use mathematical models for the analysis of queueing systems.
In~\cref{sec:queu-proc-as} we will show that the analysis of the transient behavior of even for the simplest queueing system is very complicated.
For this purpose we focus on queueing systems in steady-state.
...
To formalize the concept of stability we define in~\cref{sec:rate-stability} the notion of arrival, departure and service rate, which we use in \cref{sec:renew-reward-theor} to define the concept of \emph{load}.
We then can impose proper conditions on the load to ensure that a sensible queueing process results in steady state.
Once this is ensured, we can give meaning to a number of limiting performance measures in~\cref{sec:limits-of-emperical}.
Before introducing these definitions of rate and stability, however, we introduce notational shorthands to characterize the type of queueing process whose behavior we study \emph{in steady state}.
In~\cref{sec:graphical-summaries_1} we provide an overview of the concepts of this chapter.
