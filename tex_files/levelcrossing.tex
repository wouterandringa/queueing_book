...
Note that \Cref{fig:summaries} at the end of the chapter summarizes all concepts we develop here.
...
\end{tikzpicture}
 \caption{Plots of $Y(1,t)$ and $A(1,t)$. (For visual clarity, we subtracted $1/2$ from $A(1,t)$, for otherwise its graph would partly overlap with the graph of $Y$.)}
 \label{fig:Y_1_t}
\end{figure}
...
With $p(n)$ we define two easy, but important performance measures.
...
\caption{ For the balance equations we count how often a box around a
 state is crossed from inside and outside. In the long run the
