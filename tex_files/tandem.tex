...
Then we like to improve the one that has the most impact on the waiting times.
...
As a matter of fact, noting that the output of the first machine forms the input of the second machine, the SCV of the inter-departure times of first station must be the SCV of the inter-arrival times at the second station.
...
We remark that the liteture contains algorithms that can deal with more complicated networks of $G/G/c$ queues in which the output streams of several stations merge into the input stream of another station and rework is allowed.
In contrast, when the machines act like $M/M/c$ queues, it is possible to fully analyze the system, see~\cref{sec:jackson-networks}.

Let us write $C_{a,i}^2$ and $C_{d,i}^2$ for the SCV of the inter-arrival and inter-departure times of station $i$ in a tandem network of $G/G/c$ queues.
Then, as the output of station $i$ forms the input to station $i+1$, we have that $C_{a,i+1}^2= C_{d,i}^2$.
Thus, if we have the $C_{d,i}^2$ for all stations, we can use Sakagewa's formula to approximate the average waiting time at each station.

As it turns out, the following approximation works reasonably well for a $G/G/c$ queue: 
\begin{equation}\label{eq:57}
 C_{d,i}^2 = 1 + (1-\rho_i^2)(C_{a,i}^2-1) + \frac{\rho_i^2}{\sqrt{c_i}}(C_{s,i}^2-1).
\end{equation}
For single-server queues this becomes a bit simpler and can be given an intuitive interpretation.
...
If, however, the load is small, the server will be idle most of the time, and the inter-departure times must be approximately distributed as the inter-arrival times.
...
\begin{equation}
 \label{eq:40}
 C_{d,i}^2 =  (1-\rho_i^2) C_{a,i}^2 + \rho_i^2 C_{s,i}^2.
\end{equation}
\begin{solution}
...
In words, this gives a very useful insight: if we make service times more regular, the departure process becomes more regular. 
\begin{solution}
...
Let us apply the above to analyze the performance of a tandem netwerk of two $M/M/1$ queues.
...
Then we assume we can remove all variability at the second station; this is the best that we can possibly do.
Next, we assume we can remove all variability at the first station.
...
\begin{equation*}
 ...
 \frac12\frac{2-\rho_2}{1-\rho_2} \frac1{\mu_2}.
\end{equation*}
\begin{hint}
...
\begin{equation*}
 ...
 \frac{\rho_1}2\frac{\rho_2}{1-\rho_2} \frac1{\mu_2}
\end{equation*}
...
\begin{solution}
  ...
  By~\cref{eq:40}, $C_{d,1}^2 = (1-\rho_1)C_{a,1}^2+C_{s,1}^2= 1-\rho_1$, since by assumption $C_{a,1}^2=1$. Then,
  At the second station, $C_{a,2}^2 + C_{s,2}^2= 1-\rho_1 + 1$.
\end{solution}
\end{exercise}
