...
As a consequence, then, we can model the occurrence of an arrival in some period $i$ as a Bernoulli distributed random variable $B_i$. We assume that the $B_i, i=1,\ldots$ are independent and identically distributed (\recall{i.i.d.}) as the common random variable $B$ such that  $p=\P{B =1}$ and $1-p = \P{B=0}$. 
...
\begin{exercise} \clabel{ex:p-355}
 Show that
 \begin{equation*}
