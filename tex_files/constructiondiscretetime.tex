...
Let $L_{k-1}$ be the queue length at the end of period $k-1$, it must also be the number of customers at the start of period $k$.
...
As a first step we model the arrival process of patients as a Poisson process, cf.~\cref{sec:poisson-distribution}.
...
The queue does not explode any more, and already taking $e=1$ has a large influence.
...
\begin{exercise}[Queue with Blocking] 
A queueing system
...
\begin{exercise}[Estimating the lead time distribution] \label{ex:19} 
...
\begin{extra}[Yield loss]\label{ex:88}
...
\end{extra}
  ...
  \begin{exercise}[Priority queueing] 
...
\begin{exercise}[Queue with protected service capacity and lost capacity] 
