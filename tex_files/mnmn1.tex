...
\end{equation*}
Moreover, there is no fixed $n$ such that $n>c$. Thus, the probabilities in~\cref{eq:58} are no longer present. Thus, we see that the number of busy servers in the $M/M/\infty$ queue is
...
Observe that here we make a subtle implicit assumption; in~\ref{sec:poisson-arrivals-see} we elaborate on this assumption.
...
To relate these two `views', i.e., time-average versus observer-average, we need a new concept, \emph{PASTA}, to be developed in~\ref{sec:poisson-arrivals-see}.
