...
\begin{exercise}
Provide an interpretation of~\eqref{eq:42} in terms of a thinned Poisson arrival process.
\begin{solution}
...
\begin{exercise}\label{ex:73}
 Substitute recursion~\cref{eq:42} for $\pi(n)$ into the expression $\E{L} = \sum_{n=0}^\infty n \pi(n)$ and derive~\eqref{eq:43}.
  \begin{solution}
    We chop this up into two steps, see~\cref{ex:71} and~\cref{ex:72}.
  \end{solution}
\end{exercise}

\begin{extra}\label{ex:71}
