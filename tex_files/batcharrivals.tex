...
The left panel in~\cref{fig:levelcrossing} demonstrates the up- and down-crossings in more general terms.
The down-crossing rate is easy: there is just one arrow from right to left in~\cref{fig:levelcrossing} to down-cross level $n$, namely from $n+1$ to $n$.
Level $n$ can be up-crossed from below from many states, in fact from any level $m, 0\leq m <n$.
Below we will generalize the argument that lead~\cref{eq:22} to the level-crossing equation for the $M^X/M/1$ queue:
...
To derive~\eqref{eq:42} more formally, we turn to counting the number of up-crossings.
Define first
\begin{equation*}
...
The last step is to equate the up- and down-crossing rates, so that
 \begin{equation*}
 \sum_{m=0}^n A(m,n,t) = A(n,t) \approx  D(n,t). 
\end{equation*}
Dividing by $t$ and taking the limit $t\to\infty$ results in~\cref{eq:42}.
...
Let us use recursion~\cref{eq:42} for $\pi(n)$ to derive an expression for the expected number of units of work $\E L$ in the system.
\begin{exercise}
 Show that
...
Substitute the
  ...
  exercises of~\cref{sec:mxm1-queue:-expected}.  
\end{hint}
\begin{solution}
...
With this we can check a result of~\cref{sec:mxm1-queue:-expected}.  
\begin{exercise}
...
\begin{exercise}[\faCalculator]
  Implement the recursion~\cref{eq:42} in a computer program for the case $f(1)=f(2)=f(3)=1/3$ (recall $\P{B=k} =f_k$).
...
It is of interest to compare this result to the equation above~\cref{eq:43}.
...
As a last step, I want to check~\cref{eq:43}.
In fact, I spent at least one hour to get~\cref{eq:43} correct.
Now I can also numerically check it.
...
\begin{exercise}[Finite queueing systems] split in drieen. We consider the $M^X/M/1/K$
...
\end{enumerate}
Derive a set of recursions, analogous to~\cref{eq:42}, to compute
$\pi(n)$ for these three different acceptance rules.  
\begin{hint}
  This exercise tests your creativity and modeling skills, it is not analytically difficult.
  The best approach to problems like this is to try some simple cases first.
  For instance, consider the case with batch sizes of $1$ first, then batches of sizes $1$ or $2$, and so on.
  If system contains $K$ jobs, which batches can be accepted?
  If the system contains $K-3$, say, which batch sizes can be accepted, which will be refused, under which acceptance policy?
\end{hint}
\begin{solution}
  Let's first deal with complete rejection.
