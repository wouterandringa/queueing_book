...
\begin{exercise}
Use the renewal-reward theorem to explain this the following relation:
\begin{equation*}
...
(We choose not to specify this in terms of $N$, as not much insight will be gained from this.)

Finding the optimal $N$ is easy (from a practical point): just plot the time-average cost a function of $N$, and identify the minimum.
(It is easy to turn the above procedure into a computer program.)
It is possible to find more efficient algorithms, but we do not discuss this here.
Besides, and again from a practical point of view, it seems unlikely to use an $N$-policy when we expect that $N$ is a very large number.
Typically, the optimal $N$ will less than $100$, or so; hence, we just need to compute $100$ or so cost functions.
