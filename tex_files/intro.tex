...
Given the fraction of customers $\pi(n)$ that 'see upon arrival' the system with n customers, \footnote{$\pi(n)$ can also be interpreted as the stationary probability that an arrival sees n jobs in the system.} the manager that controls the occupation of the cashier positions is focused on keeping $\pi(4)+\pi(5)+\cdots$, i.e., the fraction of customers that see upon arrival a queue length exceeding~3, very small.
