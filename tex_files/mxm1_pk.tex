...
\begin{exercise}\clabel{ex:64}
 ...
 Compute by how much the number of items in the system would decrease if batch sizes were constant and equal to~$2$; hence the load is the same in both cases.
\begin{solution}
...
\begin{hint}
  ...
  Now consider general batch sizes $B$; realize that we sum over a random number of random variables (because $B$ is a random variable).
\end{hint}
\begin{solution}
  ...
  With this,
  \begin{align*}
    \E X & = \E{B S_1 + (B-1)S_2 + \cdots+ 1 S_B}\\
